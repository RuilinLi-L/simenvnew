...
void Estimator::_initSystem(){
    ...
    _vzFilter = new LPFilter(_dt, 3.0);
        ...
        _legOdomPub = _legOdomNh.advertise<nav_msgs::Odometry>(
            "/unitree/leg_odom", 10);
    #endif
...
void Estimator::run(){
    ...
    #ifdef COMPILE_WITH_ROS
        const int leg_odom_divider = std::max(
            1, static_cast<int>(1.0 / (_dt * _legOdomPubFreq)));
        if (_legOdomCount % leg_odom_divider == 0) {
            _legOdomMsg.header.stamp = ros::Time::now();
            _legOdomMsg.header.frame_id = "unitree_odom";
            _legOdomMsg.child_frame_id = "base";
            _legOdomMsg.pose.pose.position.x = _xhat(0);
            _legOdomMsg.pose.pose.position.y = _xhat(1);
            _legOdomMsg.pose.pose.position.z = _xhat(2);
            _legOdomMsg.pose.pose.orientation.w = _lowState->imu.quaternion[0];
            _legOdomMsg.pose.pose.orientation.x = _lowState->imu.quaternion[1];
            _legOdomMsg.pose.pose.orientation.y = _lowState->imu.quaternion[2];
            _legOdomMsg.pose.pose.orientation.z = _lowState->imu.quaternion[3];
            const Vec3 leg_velocity_body =
                _rotMatB2G.transpose() * _xhat.segment(3, 3);
            const Vec3 angular_velocity_body = _lowState->imu.getGyro();
            _legOdomMsg.twist.twist.linear.x = leg_velocity_body(0);
            _legOdomMsg.twist.twist.linear.y = leg_velocity_body(1);
            _legOdomMsg.twist.twist.linear.z = leg_velocity_body(2);
            _legOdomMsg.twist.twist.angular.x = angular_velocity_body(0);
            _legOdomMsg.twist.twist.angular.y = angular_velocity_body(1);
            _legOdomMsg.twist.twist.angular.z = angular_velocity_body(2);
            _legOdomPub.publish(_legOdomMsg);
        }
        ++_legOdomCount;
    #endif
