...
had_previous_launch=0
if pkill -f "roslaunch unitree_guide multi_floor_gazeboSim.launch" 2>/dev/null; then
  had_previous_launch=1
fi
...
# roslaunch owns the master that it auto-starts.  A just-terminated launch can
# take a moment to stop that master; starting another launch during this window
# makes the new nodes attach to a master that is about to disappear.
if [ "$had_previous_launch" -eq 1 ]; then
  echo "Waiting for the previous ROS launch to finish shutting down..."
  for _ in $(seq 1 50); do
    if ! pgrep -f "[r]oslaunch unitree_guide multi_floor_gazeboSim.launch" >/dev/null 2>&1; then
      break
    fi
    sleep 0.1
  done
...
# ROS Noetic's setup hooks read optional variables such as ROS_DISTRO before
# defining them, which is incompatible with this script's global `set -u`.
set +u
